		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '[❗] Este comando solo puede ser utilizado por un PerlaMaster del grupo!',
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        /******Entrada ApiKey******/
                        const BarBarKey = '???????SADSADASD123132**31'
...
break
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'hidetag':
       ...
       /*case 'ownergrup':
		...
		if (budy.includes(`bot`)) {
                  reply(`Soy el bot de DivaPerlas , en que te puedo ayudar `)
                  }

		if (budy.includes(`Bot traeme una hamburguesa`)) {
                  reply(`ya esta en camino ,en 20 minutos llegara por un repartidor de PedidoYa`)
                  }
	...
	if (budy.startsWith(`El bot se va`)) {
